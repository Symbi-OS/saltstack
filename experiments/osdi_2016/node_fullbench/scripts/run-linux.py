...
subprocess.call("salt -C " + SUT + " cmd.run \"killall node\"", shell=True)
subprocess.call("salt -C " + SUT + " state.apply performance", shell=True)
subprocess.call("salt -C " + SUT + " state.apply node.jsbench.linux", shell=True)
for i in range(0,100):
    out = os.open(str(i) + ".out", os.O_RDWR | os.O_CREAT)
    err = os.open(str(i) + ".err", os.O_RDWR | os.O_CREAT)
    p = subprocess.Popen("salt -C " + SUT + " state.apply node.jsbench.linux.start", shell=True, stdout=out, stderr=err)
    p.wait()
    os.close(out)
    os.close(err)
