...
salt -C "$ALL" pkg.install git refresh=True #install killall
...
salt -G $MCD state.apply $TEST memcached.${TARGET}.start -t 60 &
...
sleep 30
salt -G $MAS state.apply $TEST mutilate.start -t 6000
